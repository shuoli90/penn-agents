---
type: lecture
date: 2025-02-20T15:30:00+1:00
title: Multi-Agent, Multimodal Frameworks 
tldr: "We discuss design choices for multi-agent, multimodal frameworks."
thumbnail: /static_files/presentations/agent_3.png
hide_from_announcments: false
links: 
    - url: https://drive.google.com/file/d/1viQDocEQH-JAO4lWk6cXREozigHbx5jH/view?usp=sharing
      name: Basics
---
**Suggested Readings:**
- [Multi-Agent AI Enables Emergent Cognition and Real-Time Knowledge Synthesis in Science and Engineering](https://www.linkedin.com/pulse/multi-agent-ai-enables-emergent-cognition-real-time-science-buehler-tmtce/)
- [Building effective agents](https://www.anthropic.com/research/building-effective-agents)
- [StateFlow - Build State-Driven Workflows with Customized Speaker Selection in GroupChat](https://microsoft.github.io/autogen/0.2/blog/2024/02/29/StateFlow/)
- [Self-Reflective RAG with LangGraph](https://blog.langchain.dev/agentic-rag-with-langgraph/)
- [AutoGen: Enabling Next-Gen LLM Applications via Multi-Agent Conversation](https://arxiv.org/pdf/2308.08155)
- [Multi-Modal RAG](https://www.llamaindex.ai/blog/multi-modal-rag-621de7525fea)
- [Evaluate RAG with LlamaIndex](https://github.com/openai/openai-cookbook/blob/main/examples/evaluation/Evaluate_RAG_with_LlamaIndex.ipynb)
- [Evaluating Multi-Modal Retrieval-Augmented Generation](https://www.llamaindex.ai/blog/evaluating-multi-modal-retrieval-augmented-generation-db3ca824d428)
- [SWE-Bench](https://www.swebench.com/)
- [AppWorld: A Controllable World of Apps and People for Benchmarking Interactive Coding Agents](https://arxiv.org/pdf/2407.18901)
- [ALFWorld: Aligning Text and Embodied Environments for Interactive Learning](https://arxiv.org/pdf/2010.03768)
- [ALFRED: A Benchmark for Interpreting Grounded Instructions for Everyday Tasks](https://arxiv.org/pdf/1912.01734v2)
- [ToolBench](https://github.com/sambanova/toolbench)
- [Raising the bar on SWE-bench Verified with Claude 3.5 Sonnet](https://www.anthropic.com/research/swe-bench-sonnet)
- [WebArena](https://webarena.dev/)